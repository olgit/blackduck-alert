 ...
 * under the License.
 */
package com.synopsys.integration.alert.web.component.settings.actions;
...
import com.synopsys.integration.alert.web.component.settings.SystemMessageInitializer;
...
public class SettingsGlobalApiAction extends ApiAction {
    ...
    @Autowired
    public SettingsGlobalApiAction(final EncryptionUtility encryptionUtility, final UserAccessor userAccessor, final SystemMessageInitializer systemValidator, final SAMLManager samlManager) {
    ...
    public FieldModel afterGetAction(final FieldModel fieldModel) {
        final Optional<UserModel> defaultUser = userAccessor.getUser(SettingsUIConfig.DEFAULT_ADMIN_USER);
    ...
    private void saveDefaultAdminUserPassword(final FieldModel fieldModel) {
        ...
        if (StringUtils.isNotBlank(password)) {
            userAccessor.changeUserPassword(SettingsUIConfig.DEFAULT_ADMIN_USER, password);
        }
    }
    ...
    private void saveDefaultAdminUserEmail(final FieldModel fieldModel) {
        ...
        if (optionalEmail.isPresent()) {
            userAccessor.changeUserEmailAddress(SettingsUIConfig.DEFAULT_ADMIN_USER, optionalEmail.flatMap(FieldValueModel::getValue).orElse(""));
        }
    }
